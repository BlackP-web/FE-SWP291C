...
export default function PriceCard({
  ...
  const market = listing.marketRange
    ...
    : listing.price
    ? {
        min: Math.round(listing.price * 0.95),
        avg: listing.price,
        max: Math.round(listing.price * 1.05),
      }
    : null;
  ...
  return (
    <aside className="sticky top-24">
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-200">
        ...
        {market && (
          <PriceRangeBar min={market.min} avg={market.avg} max={market.max} />
        )}
        {listing.type === "car" && (
          <div className="mt-6 space-y-3">
            <button
              onClick={onCompare}
              className="w-full px-4 py-3 bg-black text-white rounded-xl font-semibold flex items-center justify-center gap-2"
            >
        ...
        {listing.type === "battery" && (
          <div className="mt-6 space-y-3">
            ...
            <button
              ...
              className="w-full px-4 py-3 border border-gray-300 bg-white text-gray-900 rounded-xl font-semibold flex items-center justify-center gap-2 hover:bg-gray-50"
            >
              <FileText className="w-5 h-5 text-gray-700" /> Xem hợp đồng pin
            </button>
          </div>
        )}
